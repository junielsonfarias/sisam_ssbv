...
export async function POST(request: NextRequest) {
  try {
    ...
    if (usuario.tipo_usuario !== 'administrador' && importacao.rows[0].usuario_id !== usuario.id) {
      return NextResponse.json(
        { mensagem: 'Não autorizado para retomar esta importação' },
        { status: 403 }
      )
    }

    // Verificar se pode retomar (deve estar pausado)
    if (importacao.rows[0].status !== 'pausado') {
      return NextResponse.json(
        { mensagem: `Não é possível retomar uma importação com status: ${importacao.rows[0].status}` },
        { status: 400 }
      )
    }
    ...
    return NextResponse.json({
      mensagem: 'Importação retomada com sucesso',
      status: 'processando'
    })
  } catch (error: any) {
    console.error('Erro ao retomar importação:', error)
    return NextResponse.json(
