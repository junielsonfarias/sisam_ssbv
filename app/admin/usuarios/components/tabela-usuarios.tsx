...
import { Edit, ToggleLeft, ToggleRight, Trash2, Users } from 'lucide-react'
import { Escola, Polo, Usuario, getTipoColor, getTipoLabel } from './types'
...
export function TabelaUsuarios({ usuarios, polos, escolas, onEditar, onExcluir, onToggleAtivo }: Props) {
  const getPoloNome = (id: string | null) => (id && polos.find((p) => p.id === id)?.nome) || '-'
  const getEscolaNome = (id: string | null) => (id && escolas.find((e) => e.id === id)?.nome) || '-'
    ...
    <div className="overflow-x-auto">
      <table className="w-full min-w-[320px] sm:min-w-[500px] md:min-w-[600px]">
        <thead className="bg-gray-50 dark:bg-slate-700">
          <tr>
            <th className="text-left py-3 px-4 font-semibold text-gray-700 dark:text-gray-200 text-xs uppercase tracking-wider">Usuário</th>
            <th className="text-left py-3 px-4 font-semibold text-gray-700 dark:text-gray-200 text-xs uppercase tracking-wider hidden lg:table-cell">Email</th>
            <th className="text-left py-3 px-4 font-semibold text-gray-700 dark:text-gray-200 text-xs uppercase tracking-wider">Tipo</th>
            <th className="text-left py-3 px-4 font-semibold text-gray-700 dark:text-gray-200 text-xs uppercase tracking-wider hidden md:table-cell">Vínculo</th>
            <th className="text-center py-3 px-4 font-semibold text-gray-700 dark:text-gray-200 text-xs uppercase tracking-wider">Status</th>
            <th className="text-right py-3 px-4 font-semibold text-gray-700 dark:text-gray-200 text-xs uppercase tracking-wider">Ações</th>
          </tr>
        </thead>
            ...
            <tr key={usuario.id} className={`hover:bg-gray-50 dark:hover:bg-slate-700/50 transition-colors ${!usuario.ativo ? 'opacity-60' : ''}`}>
              ...
              <td className="py-3 px-4 hidden md:table-cell">
                <span className="text-gray-600 dark:text-gray-300 text-sm">
                  {usuario.tipo_usuario === 'polo' && getPoloNome(usuario.polo_id)}
                  {usuario.tipo_usuario === 'escola' && getEscolaNome(usuario.escola_id)}
