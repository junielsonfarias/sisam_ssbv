...
interface Resumo {
  ...
  parciais: number
  dependencias: number
  dias_letivos_efetivos: number | null
  dias_letivos_exigidos: number | null
  dias_letivos_suficientes: boolean | null
}

const RESUMO_VAZIO: Resumo = {
  total: 0, aprovados: 0, reprovados: 0, em_recuperacao: 0, parciais: 0, dependencias: 0,
  dias_letivos_efetivos: null, dias_letivos_exigidos: null, dias_letivos_suficientes: null,
}
...
export default function FechamentoAnoPage() {
  ...
  const calcularResultados = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setResumo({ ...RESUMO_VAZIO, ...(data.resumo || {}) })
        setCalculado(true)
  ...
  const diasInsuficientes = resumo.dias_letivos_suficientes === false

  const aplicarResultados = async () => {
    // Override do gate de dias letivos exige justificativa (LDB art. 24).
    if (diasInsuficientes && justificativa.trim().length < 10) {
      toast.error('Informe uma justificativa (mín. 10 caracteres) para fechar com dias letivos insuficientes')
      return
    }
    ...
    try {
      const res = await fetch('/api/admin/fechamento-ano', {
        ...
        body: JSON.stringify({
          ...
          ...(diasInsuficientes ? { forcar: true, justificativa: justificativa.trim() } : {}),
        }),
      })
  ...
  const labelParecer = (parecer: string) => {
    switch (parecer) {
      case 'aprovado': return 'Aprovado'
      case 'progressao_parcial': return 'Progressão Parcial'
      case 'reprovado': return 'Reprovado'
  ...
  const corParecer = (parecer: string) => {
    switch (parecer) {
      case 'aprovado': return 'bg-green-100 text-green-800 dark:bg-green-900/40 dark:text-green-300'
      case 'progressao_parcial': return 'bg-amber-100 text-amber-800 dark:bg-amber-900/40 dark:text-amber-300'
      case 'reprovado':
      case 'recuperacao': return 'bg-red-100 text-red-800 dark:bg-red-900/40 dark:text-red-300'
  ...
  const totalDivergencias = resultados.filter(r => r.divergencia_conselho).length
    ...
    <ProtectedRoute tiposPermitidos={['administrador', 'tecnico']}>
      <div className="space-y-6">
        ...
        {carregando ? (
          ...
          <>
            {/* Banner de dias letivos (LDB art. 24, I) */}
            {resumo.dias_letivos_suficientes === false ? (
              <div className="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-xl p-4 flex items-start gap-3">
                <AlertTriangle className="w-5 h-5 text-red-600 dark:text-red-400 shrink-0 mt-0.5" />
                <div className="text-sm">
                  <p className="font-semibold text-red-800 dark:text-red-300">
                    Dias letivos insuficientes: {resumo.dias_letivos_efetivos} de {resumo.dias_letivos_exigidos} exigidos
                  </p>
                  <p className="text-red-700 dark:text-red-400 mt-0.5">
                    A LDB (art. 24, I) exige no mínimo {resumo.dias_letivos_exigidos} dias letivos. Reveja o calendário escolar
                    (feriados/recessos/reposições) ou confirme o fechamento com justificativa — o override fica registrado na auditoria.
                  </p>
                </div>
              </div>
            ) : resumo.dias_letivos_suficientes === true ? (
              <div className="bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-xl px-4 py-3 flex items-center gap-2 text-sm">
                <CheckCircle className="w-4 h-4 text-green-600 dark:text-green-400 shrink-0" />
                <span className="text-green-800 dark:text-green-300">
                  Dias letivos: <strong>{resumo.dias_letivos_efetivos}</strong> de {resumo.dias_letivos_exigidos} exigidos — calendário em conformidade (LDB art. 24).
                </span>
              </div>
            ) : (
              <div className="bg-gray-50 dark:bg-slate-800/60 border border-gray-200 dark:border-slate-700 rounded-xl px-4 py-3 flex items-center gap-2 text-xs text-gray-500 dark:text-gray-400">
                <AlertTriangle className="w-4 h-4 shrink-0" />
                Ano letivo sem datas configuradas — não foi possível validar o mínimo de dias letivos.
              </div>
            )}
                ...
                <table className="w-full divide-y divide-gray-200 dark:divide-slate-700">
                  <thead className="bg-gray-50 dark:bg-slate-700">
                    <tr>
                      ...
                      <th className="text-center py-3 px-3 text-xs font-semibold text-gray-600 dark:text-gray-300 uppercase">Conselho</th>
                    ...
                    {resultados.map((aluno, idx) => (
                      <tr key={aluno.aluno_id} className={`hover:bg-gray-50 dark:hover:bg-slate-700/50 ${idx % 2 === 0 ? '' : 'bg-gray-50/50 dark:bg-slate-800/50'}`}>
                        ...
                        <td className="py-3 px-3 text-center whitespace-nowrap">
                          {aluno.parecer_conselho ? (
                            <span
                              className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-semibold ${corParecer(aluno.parecer_conselho)}`}
                              title={aluno.divergencia_conselho
                                ? 'O parecer do Conselho de Classe diverge da proposta automática e PREVALECE sobre o cálculo (soberania do conselho).'
                                : 'Parecer do Conselho de Classe (período final).'}
                            >
                              {aluno.divergencia_conselho && <AlertTriangle className="w-3 h-3" />}
                              {labelParecer(aluno.parecer_conselho)}
                            </span>
                          ) : (
                ...
                <div className="text-xs text-gray-500 dark:text-gray-400 flex flex-wrap gap-4">
                  ...
                  {totalDivergencias > 0 && (
                    <span className="inline-flex items-center gap-1 text-amber-600 dark:text-amber-400">
                      <AlertTriangle className="w-3 h-3" /> {totalDivergencias} parecer(es) do conselho divergem da proposta
                    </span>
                  )}
                </div>
            ...
            <div className="bg-white dark:bg-slate-800 rounded-xl shadow-2xl p-6 max-w-md mx-4 space-y-4">
              ...
              {diasInsuficientes && (
                <div className="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-lg p-3 space-y-2">
                  <p className="text-xs font-semibold text-red-700 dark:text-red-300 flex items-center gap-1">
                    ...
                    Dias letivos insuficientes ({resumo.dias_letivos_efetivos}/{resumo.dias_letivos_exigidos}) — override exige justificativa
                  </p>
                  <textarea
                    value={justificativa}
                    ...
                    rows={3}
                    maxLength={500}
                    placeholder="Justifique o fechamento mesmo com dias letivos abaixo do exigido (registrado na auditoria)..."
                    className="w-full rounded-lg border border-red-300 dark:border-red-700 px-3 py-2 text-sm bg-white dark:bg-slate-700 text-gray-900 dark:text-white"
                  />
                  <p className="text-[11px] text-gray-500 dark:text-gray-400 text-right">{justificativa.trim().length}/500</p>
                </div>
              )}
              ...
              <div className="flex justify-end gap-3 pt-2">
                ...
                <button
                  ...
                  disabled={diasInsuficientes && justificativa.trim().length < 10}
