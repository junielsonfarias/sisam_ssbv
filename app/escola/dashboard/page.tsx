...
import { BarChart3, TrendingUp, GraduationCap, BookOpen, CheckCircle, XCircle, Users } from 'lucide-react'
      ...
      <LayoutDashboard tipoUsuario="escola">
        <div className="p-3 sm:p-4 md:p-6">
          <div className="mb-4 sm:mb-6 md:mb-8">
            <h1 className="text-2xl sm:text-3xl font-bold text-gray-800 dark:text-white">Dashboard da Escola</h1>
            {estatisticas.nomeEscola && (
              <p className="text-gray-600 dark:text-gray-400 mt-1 text-sm sm:text-base">
                {estatisticas.nomeEscola}
                {estatisticas.nomePolo && <span className="text-gray-500 dark:text-gray-400"> - Polo: {estatisticas.nomePolo}</span>}
              </p>
            )}
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
            <div className="bg-white dark:bg-slate-800 p-4 sm:p-6 rounded-lg shadow-md dark:shadow-slate-900/50 border-l-4 border-green-500">
              <div className="flex items-center justify-between">
                <div className="flex-1 min-w-0">
                  <p className="text-gray-600 dark:text-gray-400 text-xs sm:text-sm">Taxa de Acertos</p>
                  <p className="text-2xl sm:text-3xl font-bold text-gray-800 dark:text-white mt-1 sm:mt-2">
                    {estatisticas.taxaAcertos > 0 ? `${estatisticas.taxaAcertos.toFixed(1)}%` : '-'}
                  </p>
                </div>
                <TrendingUp className="w-10 h-10 sm:w-12 sm:h-12 text-green-600 dark:text-green-400 flex-shrink-0 ml-2" />
              </div>
            </div>
          ...
          {/* Terceira linha: Médias por Tipo de Ensino */}
