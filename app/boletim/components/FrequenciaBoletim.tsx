...
function freqColor(p: number | null) {
  ...
  if (p >= 75) return 'text-blue-800 dark:text-blue-300'
...
export default function FrequenciaBoletim({
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
      {/* Frequencia geral */}
      <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-lg border border-gray-100 dark:border-slate-700 p-6 flex flex-col items-center justify-center">
        <CalendarCheck className="w-8 h-8 text-blue-600 dark:text-blue-400 mb-3" />
        <p className={`text-5xl font-extrabold ${freqColor(frequencia_geral)}`}>
          {frequencia_geral !== null ? `${frequencia_geral}%` : '-'}
        </p>
        <p className="text-sm text-slate-500 dark:text-slate-400 mt-2">Frequencia Geral</p>
        <div className="flex items-center gap-4 mt-4 text-sm">
          <span className="text-red-500 dark:text-red-400 font-semibold">{total_faltas} faltas</span>
          {frequencia_diaria.total_dias > 0 && (
            <span className="text-slate-400 dark:text-slate-500">{frequencia_diaria.dias_presente}/{frequencia_diaria.total_dias} dias</span>
          )}
        </div>
      </div>
      ...
      <div className="lg:col-span-2 bg-white dark:bg-slate-800 rounded-2xl shadow-lg border border-gray-100 dark:border-slate-700 p-6">
        <h3 className="font-bold text-slate-800 dark:text-white flex items-center gap-2 mb-4">
          <BarChart3 className="w-5 h-5 text-blue-800 dark:text-blue-300" /> Frequencia por Periodo
        </h3>
        {frequencia.length > 0 ? (
            ...
            {frequencia.map(f => (
              <div key={f.bimestre}>
                <div className="flex items-center justify-between mb-1.5">
                  <span className="text-sm font-medium text-slate-700 dark:text-slate-300">{f.periodo_nome}</span>
                  <div className="flex items-center gap-3 text-xs text-slate-500 dark:text-slate-400">
                    <span>{f.aulas_dadas} aulas</span>
                    <span className="text-red-500 dark:text-red-400 font-medium">{f.faltas} faltas</span>
