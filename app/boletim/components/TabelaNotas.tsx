...
function notaBg(n: number | null) {
  if (n === null) return ''
  if (n >= 7) return 'bg-blue-50 dark:bg-blue-900/30'
  if (n >= 5) return 'bg-amber-50 dark:bg-amber-900/30'
...
export default function TabelaNotas({
  ...
  const nivelBadge = (nivel: string | null) => {
    ...
    const cls = nivel.includes('AVANC') ? 'bg-blue-100 text-blue-800 dark:bg-blue-900/40 dark:text-blue-300' :
      nivel.includes('ADEQU') ? 'bg-blue-50 text-blue-700 dark:bg-blue-900/30 dark:text-blue-300' :
      nivel.includes('BAS') ? 'bg-amber-100 text-amber-700 dark:bg-amber-900/40 dark:text-amber-300' :
  ...
  const presencaBadge = (p: string) => (
    <span className={`px-2 py-0.5 rounded-full text-xs font-semibold ${
      p === 'P' ? 'bg-blue-100 text-blue-900 dark:bg-blue-900/40 dark:text-blue-300' : 'bg-red-100 text-red-700 dark:bg-red-900/40 dark:text-red-300'
  ...
  return (
    <>
      ...
      <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-lg border border-gray-100 dark:border-slate-700 overflow-hidden">
        <div className="px-4 sm:px-6 py-3 sm:py-4 border-b border-gray-100 dark:border-slate-700 flex items-center gap-2">
          <BookOpen className="w-4 h-4 sm:w-5 sm:h-5 text-blue-800 dark:text-blue-300" />
          <h3 className="font-bold text-sm sm:text-base text-slate-800 dark:text-white">Notas por Disciplina</h3>
        </div>
              ...
              return (
                <div key={disc.id} className="px-4 py-3">
                  <div className="flex items-center justify-between mb-2">
                    <span className="font-semibold text-sm text-slate-800 dark:text-white">{disc.abreviacao || disc.codigo || disc.nome}</span>
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-1.5">
                    {periodos.map(p => {
                      ...
                      return (
                        <div key={p.id} className={`text-center py-1.5 rounded-lg ${temNota ? notaBg(celula?.nota_final ?? 0) : 'bg-gray-50 dark:bg-slate-700'}`}>
                          <div className="text-[9px] text-slate-400 dark:text-slate-500 font-medium">{p.numero}&#170; Av.</div>
                          <div className={`text-sm font-bold ${temNota ? notaColor(celula?.nota_final ?? 0) : 'text-gray-300 dark:text-gray-600'}`}>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-slate-50 dark:bg-slate-700/50 text-slate-500 dark:text-slate-400">
                  ...
                  <th className="text-center px-3 py-3 font-semibold whitespace-nowrap bg-slate-100 dark:bg-slate-700">Média</th>
                </tr>
              </thead>
              <tbody>
                {disciplinas.map(disc => {
                  ...
                  return (
                    <tr key={disc.id} className="border-b border-gray-50 dark:border-slate-700 hover:bg-gray-50/50 dark:hover:bg-slate-700/40">
                      <td className="px-4 py-3 font-medium text-slate-700 dark:text-slate-300 whitespace-nowrap">
                        {disc.nome}
                        <span className="ml-1 text-xs text-slate-400 dark:text-slate-500">({disc.abreviacao || disc.codigo})</span>
                      </td>
                      {periodos.map(p => {
                        const celula = notasDisc[p.numero]
                        return (
                            ...
                            {celula?.nota_final !== null && celula?.nota_final !== undefined ? (
                              <div>
                                ...
                                {celula.nota_recuperacao !== null && celula.nota_recuperacao !== undefined && (
                                  <div className="text-[10px] text-blue-500 dark:text-blue-400 mt-0.5">Rec: {celula.nota_recuperacao.toFixed(1)}</div>
                                )}
                                {celula.faltas > 0 && (
                                  <div className="text-[10px] text-red-400 dark:text-red-400 mt-0.5">{celula.faltas}F</div>
                                )}
                              </div>
                            ) : (
                      ...
                      <td className={`text-center px-3 py-3 bg-slate-50 dark:bg-slate-700/50 ${notaBg(media)}`}>
        ...
        <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-lg border border-gray-100 dark:border-slate-700 overflow-hidden">
            ...
            {avaliacoes_sisam.map((av, i) => (
              <div key={i} className="px-4 py-3 space-y-2">
                <div className="flex items-center justify-between">
                  <span className="font-semibold text-sm text-slate-800 dark:text-white">{av.avaliacao}</span>
                  <div className="flex items-center gap-2">
                ...
                <div className={`grid ${isIniciais ? 'grid-cols-2 sm:grid-cols-4' : 'grid-cols-2 sm:grid-cols-5'} gap-1.5`}>
                  <div className={`text-center py-1.5 rounded-lg ${notaBg(av.nota_lp)}`}>
                    <div className="text-[9px] text-slate-400 dark:text-slate-500 font-medium">LP</div>
                  ...
                  <div className={`text-center py-1.5 rounded-lg ${notaBg(av.nota_mat)}`}>
                    <div className="text-[9px] text-slate-400 dark:text-slate-500 font-medium">MAT</div>
                  ...
                  {!isIniciais && (
                    <div className={`text-center py-1.5 rounded-lg ${notaBg(av.nota_ch)}`}>
                      <div className="text-[9px] text-slate-400 dark:text-slate-500 font-medium">CH</div>
                  ...
                  {!isIniciais && (
                    <div className={`text-center py-1.5 rounded-lg ${notaBg(av.nota_cn)}`}>
                      <div className="text-[9px] text-slate-400 dark:text-slate-500 font-medium">CN</div>
                  ...
                  {isIniciais && (
                    <div className={`text-center py-1.5 rounded-lg ${notaBg(av.nota_producao)}`}>
                      <div className="text-[9px] text-slate-400 dark:text-slate-500 font-medium">PROD</div>
          ...
          <div className="hidden sm:block overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                {avaliacoes_sisam.map((av, i) => (
                  <tr key={i} className="border-b border-gray-50 dark:border-slate-700 hover:bg-gray-50/50 dark:hover:bg-slate-700/40">
                    <td className="px-4 py-3 font-medium text-slate-700 dark:text-slate-300">{av.avaliacao}</td>
