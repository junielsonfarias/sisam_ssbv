...
| Metrica | Valor |
|---------|-------|
| **Total de horas** | **343.5h** |
| **Dias trabalhados** | 44 |
| **Media por dia** | 7.8h |
| **Periodo** | 31/12/2025 — 17/06/2026 |
| **Total de commits** | 791 |
| **Linhas de codigo** | 93.946 (app/components/lib/database) |
| **Arquivos TS/TSX** | 1.056 |
| **Endpoints API** | 190+ (admin) + outros |
| **Testes** | 767 (40 suites) |
...
| Jun/2026 | 5.5h | 1 | 16 | Pre-producao (comparativo KontrolReq/KontrolMed + HIBP + RLS cobertura total + gerador de senha) + rate limit 2FA + **reorganizacao COMPLETA de rotas por modulo** (~73 paginas em 5 levas -> /admin/<modulo>/) + 2 fixes runtime + bug critico frequencia (presenca->status) + fase de seguranca (ModuloGuard por layout) |
...
| Pre-Producao + Rotas por Modulo (17/06/2026) | 343.5h | 791 | **Comparativo + endurecimento + reorganizacao COMPLETA de rotas**. Auditoria comparativa com KontrolReq/KontrolMed (3 agentes paralelos) -> pendencias de go-live + acertos. Seguranca: **HIBP** (senha vazada, k-anonymity, falha-aberto, 5 rotas) + **RLS cobertura total** (descoberta: tabela `usuarios` nunca teve ENABLE RLS; migration idempotente com gate de BYPASSRLS) + **gerador de senha forte** isomorfico + **rate limit por usuario no setup 2FA pre-login**. **Reorganizacao de rotas por modulo**: ~73 paginas -> /admin/<modulo>/<pagina> em 5 levas (sisam/gestor/semed/transparencia/admin), redirects 307 programaticos, dashboards de modulo renomeados; **ModuloGuard + 5 layouts** fecham bypass por URL direta (gap auditoria 30/05). 2 bugs runtime pegos em smoke test (switcher rotas antigas, imports cross-pagina) + **bug critico pre-existente**: coluna `presenca` inexistente em frequencia_diaria (6 queries, docs oficiais com freq zerada) -> status. CHECKLIST-PRODUCAO atualizado (promocao 307->308 pos-producao). Suite 751 -> 767, tsc limpo. |
