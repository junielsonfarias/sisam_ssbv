...
| **Total de commits** | 780 |
| **Linhas de codigo** | 87.890 (app/components/lib/database) |
| **Arquivos TS/TSX** | 1.050 |
...
| Jun/2026 | 3h | 1 | 5 | Pre-producao: comparativo KontrolReq/KontrolMed (3 agentes paralelos) + HIBP senha vazada + RLS cobertura total (descoberta: tabela `usuarios` nunca teve ENABLE RLS) + gerador de senha forte + CHECKLIST-PRODUCAO |
...
| Pre-Producao (17/06/2026) | 341h | 780 | **Comparativo + endurecimento de seguranca**. Auditoria comparativa com KontrolReq/KontrolMed (3 agentes paralelos) -> pendencias de go-live + acertos adotaveis. 3 frentes implementadas: **HIBP** (senha vazada, k-anonymity, falha-aberto, 5 rotas, 10 testes) + **RLS cobertura total** (descoberta critica: tabela `usuarios` nunca teve ENABLE RLS, so COMMENT; migration idempotente com gate de BYPASSRLS + script de verificacao read-only; padrao auth.uid()/SECURITY DEFINER dos outros sistemas NAO se aplica ao SISAM) + **gerador de senha forte** isomorfico (Web Crypto, botao no modal + criar-admin/init, 6 testes). **CHECKLIST-PRODUCAO.md** (6 bloqueadores; corrige falso positivo de `.env` no git). Suite 751 -> 767. tsc limpo. |
