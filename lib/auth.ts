...
export async function getUsuarioFromRequest(request: NextRequest): Promise<Usuario | null> {
  ...
  try {
    const result = await pool.query(
      'SELECT id, nome, email, tipo_usuario, polo_id, escola_id, ativo, acesso_sisam, acesso_gestor, acesso_semed, acesso_transparencia, acesso_admin FROM usuarios WHERE id = $1 AND ativo = true',
      [payload.userId]
    );
