-- ============================================
-- MIGRAÇÃO: Corrigir campo presença nas VIEWs
-- ============================================
-- O problema: MAX(presenca) retorna 'P' quando há mistura de 'P' e 'F'
-- porque 'P' > 'F' alfabeticamente. Isso faz com que alunos com falta
-- em algumas questões apareçam como presentes.
--
-- A solução: Usar MIN(presenca) para que 'F' (Faltante) tenha prioridade
-- sobre 'P' (Presente). Assim, se um aluno tem pelo menos uma falta,
-- será marcado como faltante.
-- ============================================
...
-- ETAPA 1: Recriar VIEW resultados_consolidados_v2 com MIN em vez de MAX
...
WITH notas_calculadas AS (
  SELECT
    ...
    ano_letivo,
    serie,
    -- CORREÇÃO: Usar MIN para que 'F' (falta) tenha prioridade sobre 'P' (presente)
    -- F < P alfabeticamente, então MIN retorna 'F' se existir
    MIN(presenca) as presenca,
...
-- ETAPA 2: Recriar VIEW resultados_consolidados_unificada
-- Prioriza a presença da tabela resultados_consolidados quando disponível
...
SELECT
  ...
  -- CORREÇÃO: Priorizar presença da tabela resultados_consolidados (mais confiável)
  COALESCE(rc.presenca, v2.presenca) as presenca,
...
COMMENT ON VIEW resultados_consolidados_v2 IS
  'VIEW que calcula dados consolidados dinamicamente a partir de resultados_provas. Usa MIN(presenca) para priorizar faltantes.';
