...
function addSecurityHeaders(response: NextResponse, requestId?: string, pathname?: string) {
  ...
  // HSTS — força HTTPS em produção (max-age=1 ano, incluindo subdomínios)
  ...
  // Camera: permitida em todas as rotas /admin (SPA client-side navigation compartilha headers)
  // e /terminal. Seguro pois apenas paginas de facial usam navigator.mediaDevices.
  if (isAdmin || isFacial) {
