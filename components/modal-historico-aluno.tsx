          ...
          <div className="bg-white px-6 py-4">
            ...
            {carregando ? (
              ...
              <div className="space-y-6">
                {historico.map((grupo: any, index: number) => (
                  <div key={index} className="border-b border-gray-200 pb-6 last:border-b-0 last:pb-0">
                    <div className="mb-4">
                      <h4 className="text-lg font-semibold text-gray-900">{grupo.nome}</h4>
                      {grupo.codigo && <p className="text-sm text-gray-500 font-mono">Código: {grupo.codigo}</p>}
                    </div>

                    <div className="space-y-3">
                      {grupo.registros.map((r: any, i: number) => (
                        <div key={i} className="bg-gray-50 rounded-lg p-4 border border-gray-200 hover:border-indigo-300 transition-colors">
                          ...
                          {r.resultado_id ? (
                            <div className="mt-4 pt-4 border-t border-indigo-200">
                              <div className="flex items-center gap-2 mb-3">
                                ...
                                <h5 className="text-sm font-semibold text-gray-900">Resultados da Prova</h5>
                              ...
                              {r.media_aluno && (
                                <div className="bg-gradient-to-r from-indigo-50 to-blue-50 rounded-lg p-3 border border-indigo-300">
                                  <div className="flex items-center justify-between">
                                    <div className="flex items-center gap-2">
                                      <TrendingUp className="w-5 h-5 text-indigo-600" />
                                      <p className="text-sm font-semibold text-gray-700">Média Geral</p>
                                    </div>
                                    <p className="text-2xl font-bold text-indigo-600">{formatarMedia(r.media_aluno)}</p>
                                  </div>
                                </div>
                              )}
                            </div>
              ...
              <div className="py-12 text-center">
                ...
                <p className="text-gray-500 text-lg">Nenhum histórico encontrado para este aluno.</p>
              </div>
            )}
...
function NotaCard({ titulo, nota, acertos, cor }: any) {
  ...
  return (
    <div className={`bg-white rounded-lg p-3 border ${cores[cor as keyof typeof cores]}`}>
      <p className="text-xs font-medium text-gray-500 uppercase mb-1">{titulo}</p>
      <div className="flex items-baseline gap-2">
        <p className={`text-lg font-bold ${cores[cor as keyof typeof cores]}`}>{formatarNota(nota)}</p>
