    ...
    <section id="escolas" className="py-10 sm:py-16 lg:py-20 bg-slate-50" aria-labelledby="schools-title">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center mb-6 sm:mb-12">
          <p className="text-xs sm:text-sm font-bold uppercase tracking-widest text-blue-800 mb-2 sm:mb-4">Rede de ensino</p>
          <h2 id="schools-title" className="text-2xl sm:text-4xl lg:text-5xl font-extrabold tracking-tight text-slate-900 mb-2 sm:mb-4">{title}</h2>
          <p className="text-sm sm:text-lg text-slate-500 max-w-2xl mx-auto leading-relaxed">{subtitle}</p>
        </div>

        {/* Schools Grid - 2 cols mobile, 2 tablet, 4 desktop */}
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-2 sm:gap-4 lg:gap-6">
          {displayedSchools.map((escola: any, i: number) => (
            <div
              key={escola.id || i}
              className={`group bg-white rounded-xl sm:rounded-2xl p-2.5 sm:p-5 lg:p-6 border border-slate-100 border-l-[3px] sm:border-l-4 border-l-blue-700 hover:shadow-lg hover:shadow-blue-700/5 hover:border-l-blue-800 transition-all duration-300 ${i >= 4 && !showAll ? 'hidden sm:block' : ''}`}
            >
              <div className="flex items-center sm:items-start gap-2 sm:gap-3">
                <div className="w-7 h-7 sm:w-10 sm:h-10 bg-blue-50 rounded-lg sm:rounded-xl flex items-center justify-center flex-shrink-0 group-hover:bg-blue-100 transition-colors duration-300">
                  <School className="w-3.5 h-3.5 sm:w-5 sm:h-5 text-blue-800" />
                </div>
                <div className="min-w-0">
                  <h3 className="font-bold text-[11px] sm:text-sm text-slate-900 leading-tight line-clamp-2 group-hover:text-blue-900 transition-colors duration-300">
                    {escola.nome || escola.name || 'Escola'}
                  </h3>
                  {(escola.endereco || escola.address) && (
                    <p className="hidden sm:flex items-start gap-1 text-xs text-slate-400 leading-snug mt-1.5">
        ...
        {escolas.length > 8 && !showAll && (
          <div className="flex justify-center mt-6 sm:mt-10">
            <button
              ...
              className="inline-flex items-center gap-2 px-6 sm:px-8 py-3 sm:py-4 rounded-full border-2 border-blue-200 text-blue-800 text-sm sm:text-base font-bold hover:bg-blue-50 hover:border-blue-300 transition-all duration-300"
            >
