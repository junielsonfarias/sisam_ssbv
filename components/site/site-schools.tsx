      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {displayedSchools.map((escola: any, i: number) => (
            <div
              key={escola.id || i}
              className="group bg-white rounded-2xl p-5 border border-slate-100 border-l-4 border-l-emerald-500 hover:shadow-lg hover:shadow-emerald-500/5 hover:border-l-emerald-600 transition-all duration-300"
            >
              <div className="flex items-start gap-3">
                <div className="w-10 h-10 bg-emerald-50 rounded-xl flex items-center justify-center flex-shrink-0 group-hover:bg-emerald-100 transition-colors duration-300">
                  <School className="w-5 h-5 text-emerald-600" />
                </div>
                <div className="min-w-0">
                  <h3 className="font-bold text-sm text-slate-900 leading-snug mb-1.5 line-clamp-2 group-hover:text-emerald-700 transition-colors duration-300">
                    {escola.nome || escola.name || 'Escola'}
                  </h3>
                  {(escola.polo || escola.region) && (
                    <span className="inline-block px-2 py-0.5 rounded-full bg-emerald-50 text-emerald-600 text-[10px] font-bold uppercase tracking-wider mb-1.5">
                      {escola.polo || escola.region}
                    </span>
                  )}
